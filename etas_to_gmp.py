...
#                a          b      c1      c2        d      e             sig
# PGA   rock    0.73    -7.2e-4   1.16    0.96    -1.48   -0.42           0.31
#       soil    0.71    -2.38e-3  1.72    0.96    -1.44   -2.45e-2        0.33
# PGV   rock    0.86    -5.58e-4  0.84    0.98    -1.37   -2.58           0.28
#       soil    0.89    -8.4e-4   1.39    0.95    -1.47   -2.24           0.32
#==============================================================================
#R1 = np.sqrt(R**2+9)
...
def etas2gm_intOverMag(etas_src='etas_src/kyushu_immediate2016-06-19_20:48:43_528251+00:00_xyz.xyz', fname_out='GMPE_rec.p', motion_type='PGA-soil', threshold = 0.2, maxMag = 7.0, percSource = 0.33, etas_size=None, gmp_size=None, n_procs=None, do_logz=True, fignum=None, verbose=False, year_fraction=1.0):
    ...
    if n_procs==1:
        ...
        #GMPE_rec = calc_max_GMPEs(ETAS_rec=ETAS_rec, lat_range=gmp_lat_range, lon_range=gmp_lon_range, m_reff=5.0, just_z=False)
        # Wilson: Uses forumlae for rate of exceedence of a threshold acceleration, by default we use 0.2g 
        GMPE_rec = calc_exceedance_intOverMag(ETAS_rec=forCalc_ETAS_rec, threshold=threshold, m_max=m_max, lon_range=gmp_lon_range, lat_range=gmp_lat_range, m_reff=0, just_z=False)
    #
    #t1 = time.time()
    #print(t1 - t0)
    #
...
def magIntegrand_improved(R, mc, m_max, b, threshold, motion_type='PGA-soil'):
    ...
    def f(M):
        return (GR_number_prefactor*10**(-b*M))*(0.5*(1+erf(-(threshold-f_lY_nf(M, R, motion_type))/(np.sqrt(2)*motion_type_prams[motion_type]['sig']))))
...
#
def f_lY_nf(M, R, a=None, b=None, c1=None, c2=None, d=None, e=None, sig=None, motion_type='PGA-soil'):
    # near-field corrections to Cua-Heaton PGA values
    aa = -0.882
    bb = 0.351
    if motion_type!=None:
        return f_lY_nf(M, R, motion_type=None, **motion_type_prams[motion_type])
    else:
        return (a*M + b*(np.sqrt(R**2+9) + C(M, c1, c2)) + d*np.log10(np.sqrt(R**2+9) + C(M, c1, c2)) + e) + np.log10(1+np.exp(-(R/10**(aa+bb*M))**4))
#
#@numba.jit
def C(M, c1, c2):
...
def etas_to_mag_rate(z_etas, area=100, t0=0., t1=0., t2=0., p=1.05, dm=1.0, mc=2.5):
    ...
    # 1.0/yr rate is a placeholder, we need to include annual rate in our conversion from etas->magnitude.
